  ...
  return (
    <div className="absolute inset-0 z-50" data-name="아이콘선택">
      ...
      {ICONS.map((icon, index) => {
        ...
        const leftPosition = 50 + (index * 76);

        return (
          <div
            key={icon.id}
            className="absolute size-[66px] top-[280px] cursor-pointer active:scale-95 transition-transform"
            style={{ left: `${leftPosition}px` }}
            onClick={() => handleSelect(icon.id)}
          >
            {isSelected ? (
              <>
                <div className="absolute bg-[#302b2b] border-3 border-solid border-white rounded-[8px] size-[66px]" />
                <p className="absolute inset-0 flex items-center justify-center font-['ONE_Mobile_POP_OTF:Regular',sans-serif] leading-[1.5] not-italic text-[18px] text-center text-white">
                  선택
                </p>
              </>
            ) : (
              <img alt={icon.id} className="absolute inset-0 max-w-none object-cover pointer-events-none size-full" src={icon.src} />
            )}
          </div>
        );
      })}
